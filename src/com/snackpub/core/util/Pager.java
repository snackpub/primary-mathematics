...
public class Pager<T> {
    ...
    public void setCurentPageIndex(int curentPageIndex) { // 每当页数改变，都会调用这个函数，筛选代码可以写在这里
        ...
        // 上一页，下一页确定
        prePageIndex = curentPageIndex - 1 <= 0 ? 1 : curentPageIndex - 1;
        nextPageIndex = curentPageIndex + 1;
        // 是否第一页，最后一页
        firstPage = curentPageIndex == 1;
        lastPage = curentPageIndex == pageCount;
        // 筛选工作
        smallList = new Vector();
    ...
    public Vector getBigList() {
        return bigList;
    }

    public void setBigList(Vector bigList) {
        ...
        // 筛选工作
        smallList = new Vector();
        System.out.println((curentPageIndex - 1) * countPerpage);
		if ((curentPageIndex - 1) * countPerpage > recordCount) {
			JOptionPane.showMessageDialog(null, "没用数据了！", "提示", JOptionPane.WARNING_MESSAGE);
			return;
		}
        for (int i = (curentPageIndex - 1) * countPerpage; i < curentPageIndex
                * countPerpage
    ...
    public Vector getSmallList() {
        return smallList;
    }

    public void setSmallList(Vector smallList) {
